...
from .storage import Log, StateMachine, FileStorage
from .util.timer import Timer

heartbeat_interval = 0.3
step_down_missed_heartbeats = 5
step_down_interval = heartbeat_interval * step_down_missed_heartbeats
election_interval_spread = 3
election_interval = (step_down_interval, step_down_interval * election_interval_spread)
...
def validate_term(func):
    ...
    return on_receive_function  # return wrapper
...
class Leader(BaseState):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.step_down_timer = Timer(
            step_down_missed_heartbeats * heartbeat_interval,
            self.state.to_follower,
        )
    ...
    def on_receive_append_entries_response(self, data):
        ...
        if self.log.last_log_index >= self.log.next_index[sender_id]:
            asyncio.ensure_future(
                self.append_entries(destination=sender_id), loop=self.loop
            )
    ...
    def update_commit_index(self):
        ...
        for index in range(self.log.commit_index + 1, self.log.last_log_index + 1):
            commited_count = len(
                [
                    1
                    for follower in self.log.match_index
...
class Candidate(BaseState):
    ...
    def start(self):
        ...
        self.storage.update({"term": self.storage.term + 1, "voted_for": self.id})
    ...
    def on_receive_append_entries(self, data):
        ...
        try:
            ...
            if prev_log_index > self.log.last_log_index or (
                prev_log_index
                and self.log[prev_log_index]["term"] != data["prev_log_term"]
            ):
                response = {
...
# # from my_module import Leader, Candidate, Follower
...
# def test_validate_term_current_term_is_stale(my_object):
#     my_object.storage.term = 1
#     data = {"term": 2}
#     func = MagicMock(return_value=None)

#     wrapped_func = validate_term(func)
#     wrapped_func(my_object, data)

#     assert my_object.storage.term == 2
#     assert isinstance(my_object, Follower)


# def test_validate_term_received_term_is_stale(my_object):
#     my_object.storage.term = 2
#     data = {"term": 1, "type": "some_type", "sender": "some_sender"}
#     func = MagicMock(return_value=None)

#     wrapped_func = validate_term(func)
#     response = wrapped_func(my_object, data)

#     assert my_object.storage.term == 2
#     assert func.called == False
#     assert response == {
#         "type": "some_type_response",
#         "term": 2,
#         "success": False,
#     }
